...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $sql = "INSERT INTO bookings (client_id,truck_id,origin,destination,distance_km,estimated_cost) VALUES ($client_id,$truck_id,'$origin','$destination',$distance,$cost)";
    if(mysqli_query($conn,$sql)){
        mysqli_query($conn, "UPDATE trucks SET status='booked' WHERE id=$truck_id");
        header('Location: client_dashboard.php'); exit;
    } else $error = mysqli_error($conn);
}
?>
<!doctype html><html><head><meta charset='utf-8'><title>Book Truck</title></head><body>
...
<form method="post">
  ...
  Distance (km - enter approximate):<br><input name="distance_km" type="number" step="0.1" required><br><br>
